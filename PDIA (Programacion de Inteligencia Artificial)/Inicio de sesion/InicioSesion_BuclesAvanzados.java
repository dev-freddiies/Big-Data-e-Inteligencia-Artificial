...
Scanner numUsuario = new Scanner(System.in);	// Utilizamos el scanner
System.out.println("Introduce un numero mayor que 0, por favor: ");	// Avisamos que no se permite un numero mayor que 0
int num = numUsuario.nextInt();		// Declaramos e inicializamos una variable num como entero para que sea el numero que nos estan introduciendo por teclado
 
int contador = 0;	// Creamos una variable contador de tipo int inicializandola a 0
        
if(num >= 0) {		// Validamos con un bucle if que el numero sea mayor que 0
            
	for(int i=1; i<=num; i++){		// Con un bucle for que vaya desde el 1 hasta el numero introducido
                
		contador = contador + i;	// El contador lo incrementamos a 1 (Que le vaya sumando), tambien podria ponerse como contador +=
        System.out.println("El valor total es: " + contador);		// Mostramos el resultado por pantalla y concatenandolo con el +
	...
	System.out.println("El valor introducido no es mayor a 0");	// Si es inferior a 0 hacemos que se salga del programa
